defmodule MicroblogWeb.UserView do
  ...
  def list_posts(user) do
  	Repo.all(from p in Post, where: ^user.id == p.user_id) |> Repo.preload(:user)
  end

  def list_followers(user) do
    Repo.all(from u in User, join: f in Follow, where: f.following_id == ^user.id and u.id != ^user.id, select: u)

  end

  def list_following(user) do
  	Repo.all(from u in User, join: f in Follow, where: f.follower_id == ^user.id and u.id != ^user.id, select: u)
  end
